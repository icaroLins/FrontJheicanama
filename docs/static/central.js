document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleContratar(vagaId, candidatoId) {
        ...
        try {
            const response = await fetch(`https://jheicanama-production.up.railway.app/vagas/${vagaId}/candidato/${candidatoId}/status`, {
                method: 'POST',
                headers: {
    ...
    async function handleDispensar(vagaId, candidatoId) {
        ...
        try {
            const response = await fetch(`https://jheicanama-production.up.railway.app/vagas/${vagaId}/candidato/${candidatoId}/recusar`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`, // Envia o token
                    'Content-Type': 'application/json'
    ...
    async function handleInformacoes(vagaId, cardElement) {
        ...
        try {
            const API_CANDIDATOS = `https://jheicanama-production.up.railway.app/vagas/${vagaId}/candidaturas`;
    ...
    function handleEditar(vagaId, cardElement) {
        ...
        const remuneracao = remunElem ? remunElem.textContent.replace('Remuneração: ', '').replace('R
...
, '').replace(/\./g, '').replace(',', '.') : '';
        const descricao = descElem ? descElem.textContent.replace('Descrição indisponível', '') : '';
        painel.innerHTML = `
            <div class="editor-vaga">
                <input class="input-titulo" type="text" value="${titulo}">
                <input class="input-area" type="text" value="${area}">
                <input class="input-wage" type="number" step="0.01" value="${remuneracao}">
                <textarea class="input-desc">${descricao}</textarea>
                <button class="btn-salvar-edicao">Salvar</button>
                <button class="btn-cancelar-edicao">Cancelar</button>
            </div>
        `;
        cardElement.appendChild(painel);
        painel.querySelector('.btn-cancelar-edicao').addEventListener('click', () => painel.remove());
        painel.querySelector('.btn-salvar-edicao').addEventListener('click', async () => {
            const body = {
                title: painel.querySelector('.input-titulo').value,
                area: painel.querySelector('.input-area').value,
                wage: Number(painel.querySelector('.input-wage').value),
                description: painel.querySelector('.input-desc').value
            };
            try {
                const res = await fetch(`https://jheicanama-production.up.railway.app/vaga/${vagaId}/editar`, {
                    method: 'PUT',
                    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                    body: JSON.stringify(body)
                });
                const texto = await res.text();
                if (!res.ok) throw new Error(texto || `HTTP ${res.status}`);
                if (tituloElem) tituloElem.textContent = body.title || titulo;
                if (areaElem) areaElem.textContent = `Área: ${body.area || area}`;
                const remunFormat = parseFloat(body.wage || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2 });
                if (remunElem) remunElem.innerHTML = `Remuneração: <strong>${remunFormat}</strong>`;
                if (descElem) descElem.textContent = body.description || descricao || 'Descrição indisponível';
                painel.remove();
    ...
    function handleFinalizar(vagaId, cardElement) {
        ...
        confirmPanel.querySelector('.btn-confirma').addEventListener('click', async () => {
            ...
            try {
                ...
                if (!response.ok) throw new Error(`Erro ao finalizar vaga: ${response.status}`);
